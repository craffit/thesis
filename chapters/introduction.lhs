...
\noindent Such a transformation requires a transformation of both the terms and the types. The types are changed from |Int|s to |Float|s, and the |`div`| operator is replaced by |/|. The type changes are not just local but propagate throughout the program: a program-wide type transformation. In recent years several of these type-changing program transformations have been developed, such as the worker/wrapper transformation by Gill and Hutton~\cite{gill09} or a continuation passing style transformation by Ahmed and Blume~\cite{ahmed11}. They both show that their transformations are equivalence preserving and type-preserving, two crucial properties for typed program transformations.
...
\begin{itemize}
  \item[\textbf{Migration}] A situation when program-wide modification of types is desired when migrating between libraries. In Haskell this could be migrating a program which uses |Text| as a representation for character data, to a representation using |ByteString|s. This means changing the |Text| type inside the program as much as possible, and replacing the functions that work on |Text| by functions that work on |ByteString|s. A similar migration occurs when moving between parsing libraries or when upgrading to a new version of a package.
...
Chapter~\ref{chap:tts} introduces the general concepts and construction of the TTS system. First more extensive motivating examples of type-changing program transformations are introduced in the first two sections. Subsequently the basic type-and-transform system for the simply typed lambda calculus is presented, followed by an example application of the Hughes' strings in Section~\ref{sec:hughes-transform}. The last section will briefly discuss the possibilities and problems when implementing an actual transformations system.

Chapter~\ref{chap:proof} establishes the core properties of the type-and-transform system. Section~\ref{sec:typing} and Section~\ref{sec:productivity} establish the type soundness and productivity of the type-and-transform system. Section~\ref{sec:semantics} introduces logical relations and uses these to prove the semantic correctness of program transformations. The last two sections discuss some of the theoretical background of the transformation system.

Chapter~\ref{chap:mechanical} discusses the formalization of the type-and-transform system in the programming language Agda in the first two sections and shows how the transformation system is mechanically proven correct in section~\ref{sec:mechanical-properties}. Section~\ref{sec:monoid} introduces an example transformation based on monoids, the last section discusses the content of this chapter.

In Chapter~\ref{chap:extensions} possible extensions of the type-and-transform system are discussed, to make the system ready for real-world languages. The last two chapters discuss future work and conclude.
